...
public class AttributeBucketerTest extends SearchESTest {
	...
	public void testMatches() throws Exception {
		...
		Assertions.assertThat(buckets.get(2).getBucketTerms()).containsOnly("production", "corn");
		Assertions.assertThat(buckets.get(3).getBucketTerms()).containsOnly("corn", "production planning");
		Assertions.assertThat(buckets.get(1).getBucketTerms()).containsOnly("popcorn production");
		Assertions.assertThat(buckets.get(4).getBucketTerms()).containsOnly("iron production");
		Assertions.assertThat(buckets.get(6).getBucketTerms()).containsOnly("yellow corn");
		Assertions.assertThat(buckets.get(5).getBucketTerms()).containsOnly("production planning");
	...
	public void testMatchesWithLoopCorrection() throws Exception {
		...
		List<Bucket> buckets = AttributeBucketer.createBucketList(client(), TEST_INDEX_NAME, TYPE_NAME,
				"corn production", 1, 4, LOC);
		Assertions.assertThat(buckets).hasSize(2);

		buckets = AttributeBucketer.createBucketList(client(), TEST_INDEX_NAME, TYPE_NAME, "corn", 1, 4, LOC);
	...
	public void testMatch2() throws Exception {
		...
		Assertions.assertThat(buckets.get(0).getBucketTerms()).containsExactly("wheat production", "wheat");

		Assertions.assertThat(buckets.get(1).getBucketTerms()).containsExactly("wheat production");

		Assertions.assertThat(buckets.get(2).getBucketTerms()).containsExactly("production", "wheat");

		Assertions.assertThat(buckets.get(3).getBucketTerms()).containsOnly("wheat");

		Assertions.assertThat(buckets.get(4).getBucketTerms()).containsExactly("iron production", "mining wheat");
