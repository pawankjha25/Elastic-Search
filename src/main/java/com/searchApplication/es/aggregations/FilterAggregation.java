	...
	public static AggregationBuilder getAggregation() throws Exception
	{
		try
		{
			return AggregationBuilders.nested("attributes").path("attributes")
                    ...
                    .subAggregation(AggregationBuilders.terms("attLevel").field("attributes.attribute_level").size(100)
					.subAggregation(AggregationBuilders.terms("attParent").field("attributes.attribute_parent").size(100)
                    .subAggregation(AggregationBuilders.terms("attValues").field("attributes.attribute_value.raw").size(100)))))
