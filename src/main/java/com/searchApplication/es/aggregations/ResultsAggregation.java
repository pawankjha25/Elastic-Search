		...
		try
		{
			...
			if( stratumName != null && !stratumName.isEmpty() && !stratumName.contains("*") )
			{
				return AggregationBuilders.nested("database").path("db")
						...
						.subAggregation(AggregationBuilders.terms("locationParent").field("locations.location_parent.raw").size(100)
						.subAggregation(AggregationBuilders.terms("locationname").field("locations.location_name.raw").size(100)
						.subAggregation(AggregationBuilders.terms("locationid").field("locations.series_id").size(100)
						.subAggregation(AggregationBuilders.reverseNested("locReverse")
						.subAggregation(AggregationBuilders.nested("attributes").path("attributes")
						.subAggregation(AggregationBuilders.terms("attTypes").size(100).field("attributes.attribute_name").include(stratumName).size(100)
						.subAggregation(AggregationBuilders.terms("attributesValues").size(100).field("attributes.attribute_value.raw")))))))))))));
			}
			else if( stratumName != null && !stratumName.isEmpty() && stratumName.contains("*") )
			{
				...
				return AggregationBuilders.nested("database").path("db")
						...
						.subAggregation(AggregationBuilders.reverseNested("locReverse")
						.subAggregation(AggregationBuilders.nested("attributes").path("attributes")
						.subAggregation(AggregationBuilders.terms("attTypes").size(100).field("attributes.attribute_name").size(100)
						.subAggregation(AggregationBuilders.terms("attributesValues").size(100).field("attributes.attribute_value.raw")))))))))))));
			}
