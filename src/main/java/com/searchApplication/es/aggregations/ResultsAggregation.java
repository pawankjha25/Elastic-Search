		...
		try
		{
			if( stratumName != null && !stratumName.isEmpty() && !stratumName.contains("*") )
			{
					return AggregationBuilders.nested("database").path("db")
							...
							.subAggregation(AggregationBuilders.terms("locationParent").field("locations.location_parent.raw").include(locations).size(100)
							.subAggregation(AggregationBuilders.terms("locationname").field("locations.location_name.raw").include(locations).size(100)
